...
// class에 속한 접근 제어자가 생략된 데이터 필드(멤버 변수)는 같은 패키지에 있는 다른 클래스에서 사용 가능(패키지 접근 수준)
...
class Circle {
    ...
    public Circle(int a) {      // 3. Circle 클래스 생성자 호출(실행)
        r = a;                  // 4. Circle 클래스에 속하는 데이터 필드(멤버 변수) r에 파라미터 a 할당 
    }
...
class Cylinder {
    ...
    public double getVolume() {          // 11. Cylinder 클래스에 속하는 인스턴스 메소드 "getVolume" 실행
        return c.getArea() * h;          // 12. Circle 클래스에 속하는 인스턴스 메소드 "getArea" 호출
                                         // 15. 원통 부피 리턴 (인스턴스 메소드 "getArea" 실행 결과로 리턴 받은 값(원의 넓이) * h(높이))
    }
}
...
public class CylinderVolume {
    public static void main(String args[]) {   // 1. 메소드 "main" 시작
        ...
        System.out.println(cy.getVolume());    // 10. Cylinder 클래스 객체 cy에 속한 인스턴스 메소드 "getVolume" 호출
